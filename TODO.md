...
- **#112.** Cloudflare tự chèn beacon Web Analytics ở biên nhưng CSP (`script-src 'self'`, từ #65) chặn → **3 lỗi console mỗi lần tải trang** và Web Analytics không thu được số liệu nào; đã kiểm chứng KHÔNG do repo (`grep` client/ không có, HTML origin vs CF giống hệt từng byte 25 237 B) và KHÔNG do nhóm #105-#111; cần người dùng chọn: (A) tắt Web Analytics trên dashboard — khuyến nghị, hoặc (B) thêm `static.cloudflareinsights.com` vào `scriptSrc` (phải đo cả `connectSrc`) `[Model: Sonnet 5]` — [chi tiết](docs/todo/B112-cloudflare-insights-beacon-bi-csp-chan.md)
