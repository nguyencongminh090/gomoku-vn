...
- **#81.** Vào trận giải đấu (`goToMatch`) dùng full page reload — mỗi lần đóng/mở socket lại trả giá 1 lượt session-lookup SQLite đồng bộ (blocking event loop) ở handshake; nghi ngờ cao nhất, cần đo trước khi quyết định hướng sửa `[Model: Sonnet 5]` — [chi tiết](docs/todo/B81-tournament-navigate-full-page-reload-session-lookup-blocking.md)
...
- **#85.** `savePairing()` ghi SQLite đồng bộ mỗi lần pairing đổi trạng thái, `JSON.stringify` lại toàn bộ `games`/`moves` mỗi lần — nghi ngờ thấp nhất, **cần đo thực tế trước khi sửa**, có thể đóng lại nếu không đáng kể `[Model: Sonnet 5]` — [chi tiết](docs/todo/B85-save-pairing-ghi-dong-bo-json-blob-tang-dan.md)
