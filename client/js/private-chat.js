...
import { client } from './lobby.js?v=167';
...
function renderNotifButton() {
  ...
  const setIcon = (id) => {
    if (iconUse) iconUse.setAttribute('href', 'assets/icons/phosphor-sprite.svg?v=167#' + id);
  };
...
function buildWindow(userId, name) {
  ...
  root.innerHTML = `
    <div class="pm-window__header">
      <svg class="icon pm-window__icon" aria-hidden="true"><use href="assets/icons/phosphor-sprite.svg?v=167#ph-bold-chat-circle"></use></svg>
    ...
    <div class="chat-input pm-input-row">
      ...
      <button type="button" class="pm-send-btn" title="${E().escapeAttr(t('private_chat.btn_send'))}" aria-label="${E().escapeAttr(t('private_chat.btn_send'))}">
        <svg class="icon" aria-hidden="true"><use href="assets/icons/phosphor-sprite.svg?v=167#ph-bold-paper-plane-tilt"></use></svg>
      </button>
    </div>`;
...
function renderModalList() {
  ...
  for (const u of rows) {
    ...
    if (!isSelf) {
      ...
      btn.innerHTML = '<svg class="icon" aria-hidden="true"><use href="assets/icons/phosphor-sprite.svg?v=167#ph-bold-chat-circle"></use></svg>';
      li.appendChild(btn);
