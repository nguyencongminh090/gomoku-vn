...
import { client, setHeroTab, setHeroTournamentCount } from './lobby.js?v=158';
...
function renderLiveMatchRow(match) {
  ...
  const gameIndexLabel = (match.series && match.series.seriesMode !== 'single')
    ? `<span><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=158#ph-regular-repeat"></use></svg>${t('live_matches.game_index', { n: match.series.gameIndex + 1 })}</span>`
    : '';
  return `
    <div class="live-match-row" data-live-match data-tournament-id="${escapeAttr(match.tournamentId)}" data-pairing-id="${escapeAttr(match.pairingId)}">
      ...
      <div class="live-match-row__meta">
        ${gameIndexLabel}
        <span><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=158#ph-regular-eye"></use></svg>${t('live_matches.spectators', { n: match.spectatorCount })}</span>
      </div>
    </div>
  `;
}
...
function renderCard(tournament, index) {
  ...
  let statusLine;
  if (isOrganizer) {
    statusLine = `<span class="tournament-card__status tournament-card__status--registered"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=158#ph-regular-crown-simple"></use></svg>${t('tournaments.status_organizer')}</span>`;
  } else if (isRegistered) {
    statusLine = `<span class="tournament-card__status tournament-card__status--registered"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=158#ph-regular-check-circle"></use></svg>${t('tournaments.status_registered')}</span>`;
  } else if (tournament.status === 'draft') {
    statusLine = `<span class="tournament-card__status tournament-card__status--waiting"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=158#ph-regular-user-plus"></use></svg>${t('tournaments.status_open')}</span>`;
  } else {
    statusLine = '';
  }
