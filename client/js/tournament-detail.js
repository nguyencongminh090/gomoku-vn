...
function renderHeader() {
  ...
  if (tournament.format !== 'double_elim' && tournament.totalRounds) {
    ...
    roundMeta = `<span class="detail-meta-item"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-flag-checkered"></use></svg>${t('tdetail.round_label', { n: Math.min(displayRound, tournament.totalRounds), total: tournament.totalRounds })}</span>`;
  }
  detailMetaEl.innerHTML = `
    <span class="badge badge--format">${formatLabel(tournament.format)}</span>
    <span class="badge ${badge.cls}">${badge.label}</span>
    <span class="detail-meta-item"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-users-three"></use></svg>${tournament.entries.length} ${t('tournaments.players_suffix')}</span>
    <span class="detail-meta-item"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-user-circle"></use></svg>${t('tournaments.organized_by', { name: escapeHtml(tournament.organizerName || '—') })}</span>
    ${roundMeta}
  `;
  ...
  if (tournament.status === 'cancelled') {
    detailCancelSlot.innerHTML = `
      <div class="dispute-notice"><svg class="icon" style="font-size:18px;"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-warning"></use></svg>
...
function renderActionBanner() {
  ...
  actionBannerSlot.innerHTML = `
    <div class="action-banner">
      <div class="action-banner__icon"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-bold-hourglass-medium"></use></svg></div>
      <div>
...
function renderPairingCard(pairing) {
  ...
  if (pairing.state === 'Negotiating' || pairing.state === 'Paired') {
    metaLine = `<svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-hourglass"></use></svg>${formatHoursRemaining(pairing.deadline)}`;
    ...
    if (pairing.disputed) {
      ...
      metaLine = `<svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-calendar"></use></svg>${t('tdetail.proposed_label')}: ${formatDateTime(pairing.proposedTime)} &nbsp;·&nbsp; ${formatHoursRemaining(pairing.deadline)}`;
    } else if (isMine) {
      metaLine = `<svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-calendar"></use></svg>${t('tdetail.proposed_label')}: ${formatDateTime(pairing.proposedTime)} &nbsp;·&nbsp; ${formatHoursRemaining(pairing.deadline)}`;
      ...
      metaLine = `<svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-calendar"></use></svg>${t('tdetail.proposed_label')}: ${formatDateTime(pairing.proposedTime)}`;
    }
    ...
    metaLine = `<svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-calendar"></use></svg>${formatDateTime(pairing.agreedTime)}`;
    if (pairing.rescheduleRequest) {
      metaLine += ` &nbsp;·&nbsp; <svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-arrows-clockwise"></use></svg>${t('tdetail.reschedule_pending', { name: entryName(pairing.rescheduleRequest.requestedBy), time: formatDateTime(pairing.rescheduleRequest.newTime) })}`;
      if (isOrganizer()) {
  ...
  const organizerToolsHtml = organizerActions.length
    ? `<div class="organizer-tools"><span class="organizer-tools__label"><svg class="icon"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-crown-simple"></use></svg>${t('tdetail.organizer_tools')}</span>${organizerActions.join('')}</div>`
    : '';
...
function renderCrossTable() {
  ...
  const partialNotice = tournament.status === 'cancelled'
    ? `<div class="dispute-notice"><svg class="icon" style="font-size:18px;"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-warning"></use></svg><span>${t('tdetail.standings_partial_notice')}</span></div>`
    : '';
        ...
        <tbody>
          ${entries.map((rowEntry) => {
            ...
            const trophyIcon = isChampion
              ? `<svg class="icon cross-table__trophy cross-table__trophy--champion" title="${t('tdetail.cross_table_champion')}"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-trophy"></use></svg>`
              : (isRunnerUp ? `<svg class="icon cross-table__trophy cross-table__trophy--runner-up" title="${t('tdetail.cross_table_runner_up')}"><use href="assets/icons/phosphor-sprite.svg?v=156#ph-regular-trophy"></use></svg>` : '');
            return `
              <tr class="${rowCls}">
